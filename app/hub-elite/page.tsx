...
function formatMs(ms: number) { if (!Number.isFinite(ms)) return '-'; const m = Math.floor(ms / 60000); const s = Math.floor((ms % 60000) / 1000); const ms3 = ms % 1000; return `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}.${String(ms3).padStart(3,'0')}`; }
  ...
  return (
    <main className="shell">
      <section className="hero">
        <div>
          <div className="eyebrow">TRACKMANIA ELITE HUB</div>
          <h1>Mehr Klarheit, mehr Competition, mehr Style</h1>
          <p className="sub">Optimiert mit sinnvollen Werten: Punkte, WRs, Podien, Win-Rate, Ø Gap und Gold/Silber/Bronze pro Strecke. Spielerbilder laufen über Avatar-URL oder Initialen-Fallback.</p>
          <div className="actions">
            <input list="track-list-elite" value={newTime.track} onChange={(e)=>setNewTime((v)=>({ ...v, track:e.target.value }))} placeholder="Strecke wählen oder neue eingeben" />
            <datalist id="track-list-elite">{tracks.map((t)=><option key={t} value={t} />)}</datalist>
          ...
          <div className="stats">
            ...
            <div className="stat"><span>Podiumsplätze</span><strong>{runs.length >= 3 ? ranking.reduce((a,b)=>a+b.podiums,0) : 0}</strong><small>Top-3 Auswertungen</small></div>
        ...
        <div className="card">
          ...
          <div className="legend">
            <div><strong>Punkte</strong><span>Gesamtwertung aus Platzierungen pro Strecke</span></div>
            <div><strong>WRs</strong><span>Anzahl der Strecken, auf denen der Spieler #1 ist</span></div>
            <div><strong>Podien</strong><span>Top-3 Platzierungen über alle Strecken</span></div>
            <div><strong>Ø Gap</strong><span>durchschnittlicher Abstand zur Bestzeit</span></div>
            <div><strong>Win-Rate</strong><span>Wie oft ein Fahrer bei gespielten Strecken #1 wurde</span></div>
          </div>
        </div>
      </section>

      <section className="panel">
        <div className="panel-head"><div><div className="eyebrow">GLOBAL RANKING</div><h2>Punktesystem pro Strecke</h2></div></div>
        <div className="table head seven"><span>#</span><span>Spieler</span><span>Punkte</span><span>WRs</span><span>Podien</span><span>Ø Gap</span><span>Win-Rate</span></div>
        {ranking.map((row, idx) => (
          <div className="table row seven" key={row.player}>
            <span>#{idx + 1}</span>
