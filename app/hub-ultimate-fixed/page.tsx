...
function firstMatch(headers: string[], candidates: string[]) { return headers.findIndex((header) => candidates.includes(normalizeHeader(header))); }
...
function Modal({ open, title, children, onClose }: { open: boolean; title: string; children: React.ReactNode; onClose: () => void }) {
  if (!open) return null;
  return <div className="tm-backdrop" onClick={onClose}><div className="tm-modal" onClick={(e) => e.stopPropagation()}><div className="tm-row tm-space"><div><div className="tm-eyebrow">TRACKMANIA ULTIMATE FIXED</div><h3>{title}</h3></div><button className="tm-btn tm-btn-ghost" onClick={onClose}>Schließen</button></div>{children}</div></div>;
}
...
export default function Page() {
  ...
  const recordsByTrack = useMemo(() => Array.from(groupedByTrack.entries()).map(([, arr]) => arr[0]).sort((a, b) => a.track.localeCompare(b.track)), [groupedByTrack]);
  ...
  const ranking = useMemo<RankingRow[]>(() => {
    ...
    return Array.from(score.values()).map((row) => ({ ...row, avgGap: row.entries ? row.avgGap / row.entries : Infinity })).sort((a, b) => b.points - a.points || b.records - a.records || a.avgGap - b.avgGap);
  }, [groupedByTrack]);
  ...
  const headToHead = useMemo<H2HRow[]>(() => {
    ...
    for (let i = 0; i < players.length; i++) {
      for (let j = i + 1; j < players.length; j++) {
        ...
        let winsA = 0, winsB = 0, ties = 0, tracksCount = 0;
        groupedByTrack.forEach((arr) => {
        ...
        if (tracksCount > 0) rows.push({ playerA: a, playerB: b, winsA, winsB, ties, tracks: tracksCount });
      }
    }
  ...
  return (
    <main className="tm-shell">
      <section className="tm-hero">
        <div>
          <div className="tm-eyebrow">TRACKMANIA ULTIMATE FIXED</div>
          <h1>Ranking, Head-to-Head, Dominanz und Track Details</h1>
          <p className="tm-sub">Gefixte Ultimate-Version mit funktionierendem Speichern über dieselbe Domain und sauberem Ranking aus <strong>Stammdaten_Pro</strong>.</p>
      ...
      <Modal open={timeModal} title="Neue Zeit speichern" onClose={() => setTimeModal(false)}>
        <div className="tm-form">
          ...
          <label><span>Strecke</span><input list="track-list-ultimate-fixed" value={newTime.track} onChange={(e) => setNewTime((v) => ({ ...v, track: e.target.value }))} placeholder="Strecke wählen oder neue eingeben" /><datalist id="track-list-ultimate-fixed">{tracks.map((t) => <option key={t} value={t} />)}</datalist></label>
